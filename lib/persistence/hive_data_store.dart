...
class HiveDataStore {
  ...
  static String tasksStateKey(String key) => 'tasksState/$key';
  ...
  Future<void> createDemoTasks({
    ...
    final backtBox = Hive.box<Task>(backTasksBoxName);
    if (backtBox.isEmpty || force) {
      await backtBox.clear();
      await backtBox.addAll(backTasks);
    }
  }
  ...
  Future<void> setTaskState({
    ...
    await box.put(tasksStateKey(task.id), taskState);
  }

  ValueListenable<Box<TaskState>> taskStateListenable({required Task task}) {
    final box = Hive.box<TaskState>(tasksStateBoxName);
    final key = tasksStateKey(task.id);
